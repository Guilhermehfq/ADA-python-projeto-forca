...
def testar_letra(letra_escolhida, item_aleatorio, letras_usadas, palavra_escondida, lista_erros):
    if letra_escolhida in letras_usadas:
        print(
            f'A palavra {letra_escolhida} já foi usada anteriormente, escolha novamente')
    elif letra_escolhida in item_aleatorio.lower():
        escolha_certa(letras_usadas, letra_escolhida,
                      item_aleatorio, palavra_escondida)
...
def escolha_errada(letra_escolhida, letras_usadas, lista_erros):
    letras_usadas.append(letra_escolhida)
    print(
        f'Voce errou, a palavra secreta não possui a letra {letra_escolhida}')
...
def game_over(item_aleatorio, lista_erros, continuar):
    if len(lista_erros) == 6:
        print(erro_6)
        print(
            f'Game over! Voce errou o maximo de 6 tentativas, a palavra certa era {item_aleatorio}')
        continuar = False
    else:
        continuar = True
    return continuar
...
def tentar_novamente():
    ...
    while continuar == True:
        ...
        if tentar_novamente_escolha.lower() in ['sim', 's', 'yes', 'y']:
            ...
            print(
                f'Opção invalida. Foi inserido "{tentar_novamente_escolha}", escolha entre [sim, s, yes, y] para jogar novamente ou [não, nao, n, no] para fechar o programa')
